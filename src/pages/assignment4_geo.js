...
const Charts = () => {
    ...
    const xScaleBar=[
        d3.scaleBand()
            .domain(
                data.map(d => d.station)
            )
            .range([0, innerWidth])
    ]; //replace it with the correct scale
       
    const yScaleBar=[
        d3.scaleLinear()
            .domain(
                [0, d3.max(data, d => d.start)]
            )
    ...
    const changeHandler = (event) => {
       ...
       //顶部的滑动条要正常工作，可以将其移动以选择每月数据。
       setMonth(event.target.value); // JavaScript中的setMonth()方法可以设置一个月份
